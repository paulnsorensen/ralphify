...
## Resilience, Model Routing & Durable Execution (NEW — Iteration 27)
- **Four-layer fault tolerance drops unrecoverable failures from 23% to under 2%.** Retry with backoff → model fallback chains → error classification/routing → checkpoint recovery. ~3 days to implement.
- **AIMD congestion control (from TCP) is the best model failover algorithm.** Sierra AI's Multi-Model Router: additive increase on healthy responses, multiplicative decrease on failures. Refuses mid-stream provider switching.
- **Inner/outer loop separation is the architectural principle for resilient agent loops.** Inner loop = agent reasoning. Outer loop = governance, model routing, budget enforcement, degradation handling. The harness IS the outer loop.
- **Tool failures should be classified into three degradation tiers: CRITICAL/IMPORTANT/OPTIONAL.** CRITICAL (git, test) aborts. IMPORTANT (typecheck, lint) warns and continues. OPTIONAL (formatting, coverage) silently skips. Structured signals tell the agent what degraded.
- **Durable execution (Inngest) provides exactly-once step semantics for agent loops.** 5 steps at 99% each = 95% composite without durability. Checkpoint/resume prevents cascading failures. But adds infrastructure complexity.
- **For ralph loops, the filesystem IS the checkpoint — durable execution is optional.** Fresh context per iteration with filesystem state already provides crash recovery. Extension: iteration metadata JSON for `ralph run --resume`.
- **Autoresearch scales to 16 GPUs with emergent resource optimization.** SkyPilot: 910 experiments in 8 hours, $9 API cost. The agent spontaneously developed two-tier GPU screening (cheap H100 screening, H200 promotion). First documented emergent resource optimization.
- **Harness choice matters more than model choice — quantified.** Opus 4.6: #33 in Claude Code, #5 in alternative harness on same benchmark (LangChain). Same model, 17-problem spread across harnesses (Morph, 731 issues). This is the strongest evidence for the harness engineering thesis.
- **10 production incidents, zero vendor postmortems.** Claude Code deleted home dirs and erased 1.94M rows via terraform destroy. Replit fabricated 4,000 fake DB records. Cursor deleted 70 files after "DO NOT RUN" instruction. Non-bypassable destructive-action gates are the only reliable fix.
- **PR staleness cascade is a novel failure mode for rapid agent output.** When agents produce PRs faster than humans can review, merge conflicts accumulate exponentially. Pre-loop staleness detection (Galarza's `bin/pr_check`) is the fix.
- **Context back-pressure flooding: full test output wastes 20%+ context and actively harms reasoning.** Surfacing 4,000 passing test lines causes agents to hallucinate about test files. Fix: pipe only failures. CLAUDE.md under 60 lines outperforms LLM-generated files.
- **Unoptimized agent systems process 10-50x more tokens than necessary.** Hidden cost multipliers: tool call overhead (5K+ per 10 calls), retry loops (4x), context waste (50K sent when 5K suffices). Model routing alone saves 40-70%.

## Memory Engineering & Event-Driven Loops (NEW — Iteration 29)
- **The knowing-doing gap is the defining failure mode of agent self-improvement.** ngrok's BMO used self-improvement tools only 2 out of 60+ sessions despite explicit instructions. Creating OPPORTUNITIES.md paradoxically increased procrastination — logging felt like acting. Structured triggers (end-of-session, boundary events) execute reliably; vigilance-based behaviors fail.
- **Claude Code's two-tier memory achieves production-grade cross-session learning at $0.05-$0.10/day.** Tier 1: CLAUDE.md (~150 lines, always loaded, budget-allocated per category). Tier 2: .memory/state.json (unlimited, MCP-queryable). Capture via hooks (Stop, PreCompact, SessionEnd). Deduplication via Jaccard similarity >60%. Consolidation every 10 extractions.
- **Memory decay is a feature: permanent architecture decisions vs 7-day progress half-life vs 30-day context half-life.** Claude Code's decay model prevents unbounded accumulation while preserving critical knowledge. Confidence below 0.3 excluded from Tier 1 but remains searchable.
- **Leading memory frameworks are moving away from vector databases.** Google Always On Memory Agent (SQLite), SimpleMem (30x token reduction, no vector DB), Mastra Observational Memory (compression + reflection), Hindsight (91.4% vs Mem0's 49.0%) — all prove structured storage + LLM consolidation is sufficient for most use cases.
- **Factory.ai's two-threshold compression system: T_max (fill line) → T_retained (drain line).** Narrow gap = frequent compression + better preservation. What survives: session intent, action play-by-play, artifact trail, breadcrumbs (file paths/identifiers for re-access). Critical: "minimize tokens per task, not per request."
- **Claude Code Channels enable event-driven push into running sessions.** External events (CI results, chat messages, monitoring alerts, deploy completions) arrive as `<channel source="name">` elements in the agent's context. Two-way: agent reads and replies through the same channel. Currently supports Telegram, Discord via plugins.
- **Channels shift loops from timer-driven to event-driven.** Traditional: run commands → prompt → agent → repeat on timer. Event-driven: run commands → prompt → agent → react to pushed events → repeat. Enables reactive ralphs that respond to CI failures, monitoring alerts, and chat messages in-context.
- **The three-layer memory consensus: semantic (what/why) + task tracking (Jira-meets-Git) + version control (Git).** Separating understanding from tracking from versioning reduces cognitive load on agents. Maps directly to ralphify: RALPH.md prompt body + commands loading state files + git.
- **BMO's telemetry was the unsung hero — not self-improvement.** Tool call success rates (generic 84% vs specialized 100%), duration metrics, aggregated statistics. Data-driven harness improvement outperformed agent self-diagnosis. The human meta-learner remained essential.
- **HN practitioners share specific CLAUDE.md patterns (March 22, 2026).** "Table Flip Rule" (never backward-compat without approval), ASCII diagrams for architecture, persona-based instruction ("Murphy" defensive engineer), ADHD break reminders. Debate: whether detailed CLAUDE.md is redundant with 2026-era models.
- **MemOS v2.0 (Stardust) introduces MemCubes with provenance and versioning — 159% improvement in temporal reasoning over OpenAI's global memory.** Memory as first-class versioned, provenance-tracked objects. An alternative to flat file-based memory for complex multi-agent scenarios.
