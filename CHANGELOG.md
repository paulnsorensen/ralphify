...
- **`max_turns` enforcement** — the `max_turns` frontmatter field now actively caps tool-use events per iteration. Streaming adapters that count tool uses (Claude, Codex, Copilot, opencode) are SIGTERM'd at the limit and the iteration is recorded as completed-at-cap; adapters that emit no countable events (Crush) treat the field as a no-op.
