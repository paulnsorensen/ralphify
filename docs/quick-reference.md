...
| `max_turns` | int | no | Hard cap on tool-use events per iteration. The agent is SIGTERM'd at the limit. Missing disables the cap. Only adapters that count tool uses (Claude, Codex, Copilot, opencode) enforce it. |
